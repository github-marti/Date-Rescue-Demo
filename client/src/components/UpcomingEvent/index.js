...
function UpcomingEvent() {
  ...
  useEffect(() => {
    API.getEvents(state.userid).then(results => {
      // filter events for those that are still active and not more than 3 hours in the past
      if (results.data) {
        let upcomingEvents = results.data.filter(
          el =>
            el.active &&
            Date.parse(`${el.event_date.split("T")[0]}T${el.event_time}`) +
              10800000 >
              Date.parse(new Date())
        );
        dispatch({
      ...
      {state.upcomingEvent ? (
        <>
          <EventCard
            ...
            call_time={
              state.upcomingEvent.Call
                ? state.upcomingEvent.Call.call_time
                : null
            }
            call_type={
              state.upcomingEvent.Call
                ? state.upcomingEvent.Call.call_type
                : null
            }
            callid={
              state.upcomingEvent.Call ? state.upcomingEvent.Call.id : null
            }
